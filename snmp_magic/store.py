...
from sqlalchemy import Column, JSON, event, text
...
# =============================================================================
# Database Models
# NOTE: All child foreign keys declare ondelete="CASCADE" so that SQLite's
#       PRAGMA foreign_keys=ON will automatically clean up children when a
#       parent row is deleted.  No manual child-deletion needed in endpoints.
# =============================================================================
...
class AuthToken(SQLModel, table=True):
    ...
    Only a SHA-256 hash of the raw token is stored.
    """
    id:          Optional[int]      = Field(default=None, primary_key=True)
    user_id:     int                = Field(
...
class Device(SQLModel, table=True):
    ...
    device_type: Optional[str]  = None   # classified: switch/router/printer/etc.
    first_seen:  datetime       = Field(default_factory=datetime.utcnow)
    last_seen:   datetime       = Field(default_factory=datetime.utcnow)


class DeviceSnmpProfile(SQLModel, table=True):
    """Per-device SNMP credentials (learned/saved per device)."""
    id:        Optional[int] = Field(default=None, primary_key=True)
    device_id: int           = Field(
                   foreign_key="device.id", index=True, unique=True,
                   ondelete="CASCADE"
               )
    version:    str                = "2c"
...
class GlobalSnmpProfile(SQLModel, table=True):
    """Reusable SNMP profiles for scans (not tied to a specific device)."""
    id:         Optional[int] = Field(default=None, primary_key=True)
...
class Interface(SQLModel, table=True):
    id:           Optional[int] = Field(default=None, primary_key=True)
    device_id:    int           = Field(
                      foreign_key="device.id", ondelete="CASCADE"
                  )
    if_index:     int
    name:         str
...
class LldpNeighbor(SQLModel, table=True):
    ...
    interface_id:     int           = Field(
                          foreign_key="interface.id", ondelete="CASCADE"
                      )
...
class DeviceTag(SQLModel, table=True):
    ...
    id:         Optional[int] = Field(default=None, primary_key=True)
    device_id:  int           = Field(
                    foreign_key="device.id", index=True, ondelete="CASCADE"
                )
...
class DeviceNote(SQLModel, table=True):
    """Free-form note for a device (one per device)."""
    id:         Optional[int] = Field(default=None, primary_key=True)
    device_id:  int           = Field(
                    foreign_key="device.id", index=True, unique=True,
                    ondelete="CASCADE"
...
def _set_sqlite_pragma(dbapi_connection, connection_record):
    ...
    This is what makes ON DELETE CASCADE actually fire.
    """
    try:
        ...
        pass  # non-SQLite engines are fine
...
def _ensure_seed_admin() -> None:
    """Creates default admin: snmpmagic / snmpmagic (only if no user exists)."""
    with Session(engine) as s:
        if s.exec(select(User).where(User.username == "snmpmagic")).first():
            return
...
def _count_admins(s: Session) -> int:
    return len(
        s.exec(select(User).where(User.is_admin == True, User.is_active == True)).all()
    )
...
def delete_user_safe(*, requester_id: int, target_user_id: int) -> None:
    ...
    AuthToken rows for the deleted user are removed automatically via CASCADE.
    """
    with Session(engine) as s:
...
def update_app_settings(patch: Dict[str, Any]) -> AppSettings:
    with Session(engine) as s:
        ...
        allowed = (
            "snmp_port", "snmp_timeout_ms", "snmp_retries",
...
def list_global_snmp_profiles() -> List[GlobalSnmpProfile]:
    with Session(engine) as s:
        return list(
            s.exec(select(GlobalSnmpProfile).order_by(GlobalSnmpProfile.name)).all()
        )
...
def update_global_snmp_profile(profile_id: int, patch: Dict[str, Any]) -> None:
    ...
    with Session(engine) as s:
        ...
        _apply_snmp_fields(p, p.version, patch)

        if "is_default" in patch and bool(patch["is_default"]):
            _clear_default_profiles(s, exclude_id=p.id)
            p.is_default = True
...
def delete_global_snmp_profile(profile_id: int) -> None:
    with Session(engine) as s:
        ...
        if was_default:
            remaining = s.exec(
                select(GlobalSnmpProfile).order_by(GlobalSnmpProfile.id)
            ).all()
            if remaining:
...
def get_default_global_snmp_profile() -> Optional[Dict[str, Any]]:
    with Session(engine) as s:
        p = s.exec(
            select(GlobalSnmpProfile).where(GlobalSnmpProfile.is_default == True)
        ).first()
        if not p:
            return None
...
def global_profile_to_dict(
    p: GlobalSnmpProfile, *, include_secrets: bool = False
) -> Dict[str, Any]:
    d: Dict[str, Any] = {
        "id":         p.id,
        "name":       p.name,
...
def _apply_snmp_fields(
    p: GlobalSnmpProfile | DeviceSnmpProfile,
    version: str,
    data: Dict[str, Any],
) -> None:
    """Writes SNMP fields onto a profile object, clearing the opposite version."""
    if version == "2c":
        ...
        if "username" in data:
            ...
            p.username = u
        for k in (
            "security_level", "auth_protocol", "auth_password",
            "priv_protocol", "priv_password", "context_name",
        ):
            if k in data:
...
def _clear_default_profiles(s: Session, *, exclude_id: Optional[int] = None) -> None:
...
def upsert_device_snmp_profile(
    s: Session, ip: str, profile: Dict[str, Any], now: Optional[datetime] = None
) -> None:
    ...
    p = s.exec(
        select(DeviceSnmpProfile).where(DeviceSnmpProfile.device_id == dev.id)
    ).first()
    if not p:
    ...
    p.updated_at = now
    _apply_snmp_fields(p, p.version, profile)
    s.commit()


def get_device_snmp_profile_by_ip(
    s: Session, ip: str
...
# =============================================================================
# Device deletion (cascade handled by DB — this is just the public API)
# =============================================================================

def delete_device_by_ip(ip: str) -> bool:
    """
    Delete a device and all its children (profiles, interfaces, LLDP neighbors,
    tags, notes) in one shot via ON DELETE CASCADE.
...
def delete_device_by_id(device_id: int) -> bool:
    """
    Delete a device by primary key.  Children removed via CASCADE.
    Returns True if deleted, False if not found.
    """
    with Session(engine) as s:
...
_AUTH_MAP: Dict[Optional[str], Any] = {
    None:     None,
...
_PRIV_MAP: Dict[Optional[str], Any] = {
    None:     None,
...
def _snmp_get_sysname_sync(
    *,
    ...
    """Runs in a worker thread. Never raises — returns {success, sys_name?, error?}."""
    if not _PYSNMP_OK:
    ...
    try:
        ...
        if version == "2c":
            ...
            auth_key  = (profile.get("auth_protocol") or "").upper() or None
            priv_key  = (profile.get("priv_protocol") or "").upper() or None
            auth = UsmUserData(
                userName=username,
                authKey=profile.get("auth_password") or None,
                privKey=profile.get("priv_password") or None,
                authProtocol=_AUTH_MAP.get(auth_key),
                privProtocol=_PRIV_MAP.get(priv_key),
            )
...
def save_scan(
    ...
    # Save the Scan record in its own session, close it cleanly
    with Session(engine) as s:
        sc = Scan(
            started_at=now,
            finished_at=now,
    ...
    # Upsert devices in fresh sessions — avoids post-commit expiry issues
    if status == "ok":
        if mode == "single":
            _upsert_host_detailed(payload.get("host") or {}, now)
        elif mode == "discovery":
            for h in payload.get("hosts", []):
                _upsert_host_basic(h, now)

    return scan_id


def _upsert_host_basic(data: dict, now: datetime) -> None:
    from snmp_magic.device_classify import classify
    from snmp_magic.device_events import diff_device, record_events
    ...
    with Session(engine) as s:
        ...
        if data.get("sys_name"):
            dev.sys_name = data["sys_name"]
        if data.get("sys_descr"):
            dev.sys_descr = data["sys_descr"]
        # Classify
        cls = classify(sys_descr=dev.sys_descr, sys_name=dev.sys_name)
        dev.device_type = cls.device_type
        s.commit()


def _upsert_host_detailed(data: dict, now: datetime) -> None:
    from snmp_magic.device_classify import classify
    from snmp_magic.device_events import diff_device, record_events
    ...
    with Session(engine) as s:
        ...
        # Grab previous scan payload for diffing BEFORE we overwrite anything
        prev_scan = s.exec(
            select(Scan)
            .where(Scan.target == ip)
            .where(Scan.mode == "single")
            .order_by(Scan.id.desc())
            .limit(1)
        ).first()
        prev_payload = None
        if prev_scan and prev_scan.result:
            prev_payload = (prev_scan.result.get("host")
                            or prev_scan.result)
        ...
        # Change detection — diff prev vs current payload
        events = diff_device(ip, prev_payload, data, now)
        if events:
            record_events(s, events)
            s.commit()

        # Auto-save SNMP creds if present in scan payload
        snmp_block = data.get("snmp")
        if isinstance(snmp_block, dict) and snmp_block:
            upsert_device_snmp_profile(s, ip, snmp_block, now=now)

        # Replace interfaces: delete old ones (LldpNeighbor rows cascade automatically)
        for iface in s.exec(
        ...
        for row in data.get("interfaces", []):
            ...
            neighbors = (
                raw_lldp.get(iface.name)
                or raw_lldp.get(str(iface.if_index))
                or []
            )
            for n in neighbors:
                s.add(
...
def list_device_tags(ip: str) -> List[str]:
    with Session(engine) as s:
        ...
        tags = s.exec(
            select(DeviceTag)
            .where(DeviceTag.device_id == dev.id)
    ...
    with Session(engine) as s:
        ...
        exists = s.exec(
            select(DeviceTag).where(
                DeviceTag.device_id == dev.id, DeviceTag.name == tag
            )
        ).first()
        if exists:
            return
...
def get_device_note(ip: str) -> str:
    with Session(engine) as s:
        ...
        if not dev:
            return ""
        n = s.exec(
            select(DeviceNote).where(DeviceNote.device_id == dev.id)
        ).first()
...
def set_device_note(ip: str, note: str) -> None:
    ...
    with Session(engine) as s:
        ...
        if not row:
            row = DeviceNote(
                device_id=dev.id, note=note, updated_at=datetime.utcnow()
            )
            s.add(row)
        else:
            row.note = note
